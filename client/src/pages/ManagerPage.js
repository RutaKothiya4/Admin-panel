...
export default function ManagerPage() {
  ...
  const deleteManager = useCallback(async () => {
    ...
    try {
      ...
      setData((prev) => prev.filter((mgr) => (mgr._id || mgr.id) !== id));
    } catch (e) {
  ...
  const handleRevokeSession = async () => {
    ...
    try {
      ...
      setData((prev) =>
        prev.map((mgr) =>
          (mgr._id || mgr.id) === userId
            ? { ...mgr, refreshTokens: res.data.sessions }
            : mgr
        )
      );
    } catch (e) {
  ...
  return (
    <div className="container">
      <h1 className="text-center mb-4 text-bg-success">Managers</h1>

      <div className="m-auto w-75">
        {Array.isArray(data) && data.length > 0 ? (
          <ul className="list-group">
            {data.map((manager, index) => {
              const id       = manager?._id || manager?.id || null;
              const key      = id || index;
              const sessions = manager.refreshTokens || [];

              return (
                <li key={key} className="list-group-item">
                  {/* Manager row */}
                  <div className="d-flex justify-content-between align-items-center mb-2">
                    <span className="w-50">{manager.username}</span>
                    <button
                      className="btn btn-sm btn-danger"
                      disabled={busy}
                      onClick={() =>
                        id ? openDelModal(id) : alert("Manager ID missing.")
                      }
                    >
                      Delete
                    </button>
                        ...
                        ({ sessionId, userAgent, ipAddress, expiresAt }) => (
                          ...
                          >
                            ...
                            <button
                              ...
                              disabled={busy}
                              title={`Revoke session for ${userAgent}`}
          ...
          <p className="text-center">No manager users found.</p>
        )}
      </div>
